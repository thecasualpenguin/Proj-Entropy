...
class BitrateStats:
    ...
    def _assemble_bitrate_statistics(self):
        ...
        ret = {
            ...
            "frame_size(in bit)":[
                round(f, self.rounding_factor) for f in self._calculate_framesize()
            ],
        }
    ...
    def _print_csv(self):
        ...
        filename = filename if self.custom_filename == None else self.custom_filename

        savePath = self.custom_output_dir
        #savePath = ""
        export_path = os.path.join(savePath, filename + ".csv")
...
def extract_bitrate_python_interface(_input, custom_output_dir = None, custom_filename = None,
